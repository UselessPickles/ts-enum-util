...
describe("EnumWrapper: string enum", () => {
    ...
    test("getNameOrDefault()", () => {
        ...
        expect(enumWrapper.getNameOrDefault(-1)).toBe(undefined);
        expect(enumWrapper.getNameOrDefault(-1, "A")).toBe("A");
        expect(enumWrapper.getNameOrDefault(-1, "foo")).toBe("foo");
    });
});
